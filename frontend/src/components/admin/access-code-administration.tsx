...
import { Copy, KeyRound, RefreshCw, Search, ShieldX, Trash2 } from 'lucide-react';
...
export function AdminHomePage() {
  ...
  const filteredCodes = useMemo(() => {
    ...
    return (codes.data ?? []).filter((code) =>
      [code.orderId, code.id, codeStatus(code), code.revocationReason, code.courseId].some(
  ...
  const createTtaCodes = useMutation({
    ...
    onSuccess: (result) => {
      void refresh();
      toast.success(`${result.quantity} TTA student codes added to the ${result.sheetTab} sheet`);
    },
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : 'TTA code generation failed'),
  });

  const revokeAllCodes = useMutation({
    mutationFn: (reason: string) => revokeAllActiveAdminAccessCodes(reason, getToken),
    onSuccess: (result) => {
      void refresh();
      toast.success(`${result.revokedCount} active access codes revoked`);
    },
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : 'Bulk revocation failed'),
  });

  const deleteRevokedCodes = useMutation({
    mutationFn: (reason: string) => deleteRevokedAdminAccessCodes(reason, getToken),
    onSuccess: (result) => {
      void refresh();
      toast.success(`${result.deletedCount} revoked code records deleted`);
    },
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : 'Revoked-code cleanup failed'),
  });

  const resetTtaNumbering = useMutation({
    mutationFn: (reason: string) => resetTtaOrderNumbering(reason, getToken),
    onSuccess: (result) => toast.success(`TTA numbering reset; next ID is ${result.nextOrderId}`),
    onError: (error) =>
      toast.error(error instanceof Error ? error.message : 'Numbering reset failed'),
  });
  ...
  const confirmRevokeAll = () => {
    if (!window.confirm('Revoke every active, unredeemed access code? This cannot be undone.')) {
      return;
    }
  ...
  const confirmResetTtaNumbering = () => {
    if (
      !window.confirm('Reset TTA order numbering so the next generated ID is TTA-STUDENT-0001?')
    ) {
      return;
    }
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Generate Course 2 codes and add them to the TTA Codes tab for customer service.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <form
              ...
              >
                <div className="space-y-1.5">
                  <label className="text-sm font-medium" htmlFor="tta-code-quantity">
                    Number of codes
                  </label>
                  <Input
        ...
        <Card className="flex h-[calc(100vh-10rem)] min-h-[28rem] max-h-[52rem] flex-col overflow-hidden">
            ...
            <div className="flex flex-wrap items-start justify-between gap-3">
              ...
              <div className="flex flex-wrap gap-2">
                ...
                >
                  ...
                  {deleteRevokedCodes.isPending ? 'Deleting…' : 'Delete revoked records'}
                </Button>
                <Button
                  variant="destructive"
                  size="sm"
                ...
                >
                  ...
                  {revokeAllCodes.isPending ? 'Revoking…' : 'Revoke all active codes'}
                </Button>
              </div>
            </div>
            <CardDescription>
              {filteredCodes.length} of {codes.data?.length ?? 0} recent codes shown.
            </CardDescription>
          </CardHeader>
          <CardContent className="flex min-h-0 flex-1 flex-col gap-3">
            ...
            <div className="min-h-0 flex-1 space-y-3 overflow-y-auto pr-2">
              ...
              {codes.isError && (
                <p className="text-sm text-destructive">Unable to load access codes.</p>
              )}
