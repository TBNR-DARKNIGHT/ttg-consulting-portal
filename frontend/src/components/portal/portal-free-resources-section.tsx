...
export function PortalFreeResourcesSection() {
  return (
    <section
      className="border-b border-brand-dark/15 bg-brand-grey"
      aria-labelledby="portal-free-resources-heading"
    >
      <div className="mx-auto max-w-[960px] px-5 py-16 md:px-8 md:py-20">
        ...
        <div className="grid overflow-hidden rounded-xl border border-brand-dark/15 md:grid-cols-3">
          {resources.map(({ badge, title, description, Icon }) => (
            <article
              key={title}
              className="border-b border-brand-dark/15 bg-white px-8 py-9 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0"
            >
              <div className="grid size-11 place-items-center rounded-[10px] bg-[#e8e8fa]">
                <Icon className="size-5 text-brand-indigo" aria-hidden />
              </div>
              <span className="mt-4 inline-block rounded bg-[#e8e8fa] px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.1em] text-brand-indigo">
                {badge}
              </span>
