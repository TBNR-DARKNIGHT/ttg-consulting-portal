...
const sessions = [
  [
    '01',
    'How DSA Interviews Actually Work',
    'Understand what selection panels assess, how interviews are structured, and what strong candidates do differently.',
  ],
  [
    '02',
    'Structuring Answers That Land',
    'Build answers with substance and clarity without sounding scripted or rehearsed.',
  ],
  [
    '03',
    'Thinking Under Pressure',
    'Learn practical tools for handling unexpected questions and recovering composure.',
  ],
  [
    '04',
    'Guided Mock Interview Practice',
    'Apply every skill in realistic practice with direct, specific coaching from our team.',
  ],
] as const;
      ...
      <main>
        <section className="relative overflow-hidden bg-brand-dark">
          ...
          <div className="relative mx-auto max-w-[960px] px-5 py-16 md:px-8 md:py-20">
            ...
            <h1 className="max-w-[780px] font-serif text-4xl font-bold leading-[1.12] text-white md:text-[56px]">
              ...
              <br />
              <em className="font-normal text-[#a8a8f0]">With Confidence.</em>
            </h1>
            <p className="mt-5 max-w-[620px] font-light leading-[1.7] text-white/65">
              Two successful runs. A focused half-day format. Real coaching from mentors who have
              navigated competitive admissions across global institutions. Registration for the next
              cycle opens early 2027.
            </p>
            <a
              href={WAITLIST_EMAIL}
            ...
            {HEADER_IMAGE ? (
              <img
                src={HEADER_IMAGE}
                alt="DSA Interview Intensive participants"
        ...
        <section className="border-b border-brand-dark/15 bg-brand-grey">
          <div className="mx-auto max-w-[960px] px-5 py-12 md:px-8">
            <p className="mb-6 text-[11px] font-semibold uppercase tracking-[0.14em] text-brand-indigo">
              Programme Details
            </p>
            <div className="grid overflow-hidden rounded-xl border border-brand-dark/15 bg-white sm:grid-cols-3">
              {[
                ['Format', 'Small group interactive workshop'],
                ['Duration', '4 hours, including a short break'],
                ['Programme cycle', 'April and May annually'],
              ].map(([label, value]) => (
                <div
                  key={label}
                  className="border-b border-brand-dark/15 px-7 py-6 last:border-b-0 sm:border-b-0 sm:border-r sm:last:border-r-0"
                >
                  <p className="text-[10px] font-semibold uppercase tracking-[0.1em] text-brand-indigo">
                    {label}
                  </p>
                  <p className="mt-2 text-sm text-brand-dark/70">{value}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <section className="border-b border-brand-dark/15 bg-brand-cream">
          <div className="mx-auto max-w-[960px] px-5 py-16 md:px-8 md:py-20">
            ...
            <h2 className="text-3xl font-bold leading-[1.2] text-brand-dark md:text-[40px]">
              Four sessions.
              <br />
              <em className="font-normal text-brand-indigo">One complete preparation.</em>
            </h2>
            <div className="mt-12 grid overflow-hidden rounded-xl border border-brand-dark/15 md:grid-cols-2">
              {sessions.map(([number, title, body]) => (
                <article
                  key={number}
                  className="border-b border-brand-dark/15 bg-white px-8 py-8 odd:md:border-r [&:nth-child(3)]:md:border-b-0 [&:nth-child(4)]:border-b-0"
                >
                  <p className="font-serif text-3xl font-bold text-[#e8e8fa]">{number}</p>
                  <h3 className="mt-2 text-lg font-bold text-brand-dark">{title}</h3>
                  <p className="mt-2 text-sm leading-[1.65] text-brand-dark/70">{body}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        {SESSION_IMAGE ? (
          <div className="border-b border-brand-dark/15 bg-brand-cream px-5 py-12">
            <img
              src={SESSION_IMAGE}
              alt="Students practising DSA interviews"
        ...
        <section className="border-b border-brand-dark/15 bg-brand-grey">
          <div className="mx-auto max-w-[960px] px-5 py-16 md:px-8 md:py-20">
            ...
            <h2 className="text-3xl font-bold leading-[1.2] text-brand-dark md:text-[40px]">
              Not just prepared.
              <br />
              <em className="font-normal text-brand-indigo">Genuinely ready.</em>
            </h2>
            <div className="mt-10 grid gap-2 sm:grid-cols-2">
              {outcomes.map((outcome) => (
                <div
                  key={outcome}
                  className="flex gap-4 bg-white px-6 py-5 text-sm leading-[1.6] text-brand-dark/70"
                >
        ...
        <section className="bg-brand-dark">
          <div className="mx-auto max-w-[960px] px-5 py-16 md:px-8 md:py-20">
            ...
            <h2 className="text-3xl font-bold leading-[1.2] text-white md:text-[40px]">
              The 2026 intake is now closed.
              <br />
              <em className="font-normal text-[#a8a8f0]">Join the 2027 waitlist.</em>
            </h2>
            <p className="mt-5 max-w-[640px] leading-[1.7] text-white/55">
              Places are kept small deliberately so every student receives individual coaching and
              meaningful practice time.
            </p>
            <div className="mt-8">
              <a
                href={WAITLIST_EMAIL}
                className="inline-block rounded-[7px] bg-brand-indigo px-7 py-3.5 text-sm font-semibold text-white"
              >
                Join the 2027 Waitlist
              </a>
            </div>
        ...
        <section className="bg-brand-cream">
          <div className="mx-auto max-w-[960px] px-5 py-16 md:px-8 md:py-20">
            ...
            <h2 className="text-3xl font-bold leading-[1.2] text-brand-dark md:text-[40px]">
              ...
              <br />
              <em className="font-normal text-brand-indigo">We have other pathways.</em>
            </h2>
            <div className="mt-10 grid overflow-hidden rounded-xl border border-brand-dark/15 md:grid-cols-2">
              {[
                ['The Beyond Grades Portal', 'Open resources and self-paced courses.', '/portal'],
                [
                  'DSA Consulting',
                  'A personalised strategy built around your child’s profile.',
                  '/consult',
                ],
              ].map(([title, body, to]) => (
                <article
                  key={title}
                  className="border-b border-brand-dark/15 bg-white px-8 py-8 last:border-b-0 md:border-b-0 md:border-r md:last:border-r-0"
                >
                  <h3 className="text-lg font-bold text-brand-dark">{title}</h3>
                  <p className="mt-3 text-sm text-brand-dark/70">{body}</p>
                  <Link
                    to={to}
